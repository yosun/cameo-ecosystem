...
export default function CreatorOnboardingComplete() {
  ...
  useEffect(() => {
    const checkOnboardingStatus = async () => {
      try {
        // Get creator ID from URL params or session storage
        const creatorId = searchParams.get('creator_id') || localStorage.getItem('onboarding_creator_id');
        
        if (!creatorId) {
          setStatus('error');
          setMessage('Creator ID not found. Please try the onboarding process again.');
          return;
        }

        // Check if onboarding is complete
        const response = await fetch(`/api/stripe/connect/creator?creatorId=${creatorId}`);
        ...
        if (data.onboardingComplete) {
          setStatus('success');
          setMessage('Your payment account has been successfully set up! You can now receive royalties from your LoRA-generated content.');
          
          // Clean up session storage
          localStorage.removeItem('onboarding_creator_id');
